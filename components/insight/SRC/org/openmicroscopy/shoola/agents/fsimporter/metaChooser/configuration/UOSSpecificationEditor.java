...
import org.openmicroscopy.shoola.agents.fsimporter.metaChooser.configuration.UOSProfileEditorUI.ConfigurationPanel;
...
public class UOSSpecificationEditor extends JDialog implements ActionListener
{
	...
	private void initGUI()
	{
		...
		JTextField micName=new JTextField(reader.getMicName());
...
//		pane.add(new SpecificationPanel(reader,"Image Modul"));
		pane.add(new SpecificationPanel(reader.getObjectives(),"Objective Modul"));
		pane.add(new SpecificationPanel(reader.getDetectors(),"Detector Modul"));
		pane.add(new SpecificationPanel(reader.getLightSources(),"LightSource Modul"));

//		pane.add(new SpecificationPanel(prop.getChannelConf(),"Channel Modul"));
		pane.add(new SpecificationPanel(reader.getLightPathFilters(),"LightPath Modul"));
...
//			applyConfigurations(prop);
	...
	class SpecificationPanel extends JPanel
	{
		...
		public SpecificationPanel(List<Filter> list,String name)
		{
			setLayout(new BorderLayout());
			...
			int i=0;
			for(Filter f:list){
				JTable table=new SpecificationTable();
...
//			List list=Arrays.asList(t.isVisible(),t.getTagLabel().getText(),t.getTagValue(),t.getTagUnit().getSymbol());
			...
			list.add(t.getTagUnit().getSymbol());
			data.add(list);
